...
/**
 * Provides various filesystem operations common to scripting languages such as
 * Perl and Python but not present (currently) in the Java standard libraries.
 * 
 * @author Spence Green
 * 
 */
public final class FileSystem {
	...
	/**
	 * Copies a file. The ordering of the parameters corresponds to the Unix cp
	 * command.
	 * 
	 * @param sourceFile
	 *            The file to copy.
	...
	/**
	 * Similar to the unix gzip command, only it does not delete the file after
	 * compressing it.
	 * 
	 * @param uncompressedFileName
	 *            The file to gzip
	...
	public static void gzipFile(File uncompressedFileName,
			File compressedFileName) throws IOException {
	...
	/**
	 * Returns whether a file object both exists and has contents (i.e. the size
	 * of the file is greater than 0)
	 * 
	 * @param file
	...
	public static void main(String[] args) {
		...
		try {
			copyFile(new File(testFileName), new File(testDirName + "/"
					+ testFileName));
		} catch (IOException e) {
		...
		try {
			...
			Process p = r.exec(String.format("tar -cf %s.tar %s", testDirName,
					testDirName));
		...
		try {
			gzipFile(new File(String.format(testDirName + ".tar")), new File(
					testDirName + ".tar.gz"));
		} catch (IOException e) {
