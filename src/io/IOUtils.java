...
public class IOUtils {
	...
	/**
	 * Write object to a file with the specified name. The file is silently
	 * gzipped if the filename ends with .gz.
	 * 
	 * @param o
	...
	/**
	 * Write an object to a specified File. The file is silently gzipped if the
	 ...
	 * @return File containing the object
	 */
	public static File writeObjectToFile(Object o, File file, boolean append)
			throws IOException {
	...
	// ++ todo [cdm, Aug 2012]: Do we need the below methods? They're kind of
	// weird in unnecessarily bypassing using a Writer.
	 ...
	 *            The encoding to encode in
	 * */
	public static void writeStringToFileNoExceptions(String contents,
	 ...
	 */
	public static File writeStringToTempFile(String contents, String path,
			String encoding) throws IOException {
		OutputStream writer;
	...
	/**
	 * Writes a string to a temporary file with UTF-8 encoding, squashing
	 * exceptions
	 * 
	 * @param contents
	 *            The string to write
	...
	// todo [cdm, Sep 2013]: Can we remove this next method and its friends?
	// (Weird in silently gzipping, overlaps other functionality.)
	/**
	 * Read an object from a stored file. It is silently ungzipped, regardless
	 * of name.
	 * 
	 * @param file
	...
	public static DataInputStream getDataInputStream(
			String filenameUrlOrClassPath) throws IOException {
		return new DataInputStream(
				getInputStreamFromURLOrClasspathOrFileSystem(filenameUrlOrClassPath));
	}
	...
	/**
	 * Read an object from a stored file. The file can be anything obtained via
	 * a URL, the filesystem, or the classpath (eg in a jar file).
	 * 
	 * @param filename
	 *            The file pointing to the object to be retrieved
	 * @throws IOException
	 ...
	 * @return The object read from the file.
	 */
	public static <T> T readObjectFromURLOrClasspathOrFileSystem(String filename)
			throws IOException, ClassNotFoundException {
		ObjectInputStream ois = new ObjectInputStream(
				getInputStreamFromURLOrClasspathOrFileSystem(filename));
		Object o = ois.readObject();
		ois.close();
		return ErasureUtils.uncheckedCast(o);
	}
	...
	public static <T> T readObjectFromFile(String filename) throws IOException,
			ClassNotFoundException {
		return ErasureUtils
				.uncheckedCast(readObjectFromFile(new File(filename)));
	}
	...
	/**
	 * Locates this file either in the CLASSPATH or in the file system. The
	 * CLASSPATH takes priority.
	 * 
	 * @param name
	...
	private static InputStream findStreamInClasspathOrFileSystem(String name)
		...
		InputStream is = IOUtils.class.getClassLoader().getResourceAsStream(
				name);
		// windows File.separator is \, but getting resources only works with /
		if (is == null) {
			is = IOUtils.class.getClassLoader().getResourceAsStream(
					name.replaceAll("\\\\", "/"));
		}
	 ...
	 *         {@link IOUtils#getBufferedReaderFromClasspathOrFileSystem(String)}
	 ...
	 * @throws IOException
	 */
	public static InputStream getInputStreamFromURLOrClasspathOrFileSystem(
			String textFileOrUrl) throws IOException {
		InputStream in;
	...
	/**
	 * Open a BufferedReader to a File. If the file's getName() ends in .gz, it
	 ...
	 * 
	 * @param encoding
	 *            CharSet encoding. Maybe be null, in which case the platform
	 ...
	 * String starts with https?://, then it is first tried as a URL, otherwise
	 * it is next tried as a resource on the CLASSPATH, and then finally it is
	 * tried as a local file or other network-available file. If the String ends
	 * in .gz, it is interpreted as a gzipped file (and uncompressed). The file
	 * is then interpreted as a utf-8 text file.
	 * 
	 * @param textFileOrUrl
	 *            What to read from
	 * @return The BufferedReader
	 ...
	 * String starts with https?://, then it is first tried as a URL, otherwise
	 * it is next tried as a resource on the CLASSPATH, and then finally it is
	 * tried as a local file or other network-available file . If the String
	 * ends in .gz, it is interpreted as a gzipped file (and uncompressed), else
	 * it is interpreted as a regular text file in the given encoding.
	 * 
	 * @param textFileOrUrl
	 *            What to read from
	 * @param encoding
	 ...
	 *            The file whose lines are to be read.
	 * @param encoding
	 *            The encoding to use when reading lines.
	 * @return An Iterable containing the lines from the file.
	 */
	 ...
	 *            The class to wrap the InputStream with, e.g. GZIPInputStream.
	 *            Note that the class must have a constructor that accepts an
	 *            InputStream.
	 * @return An Iterable containing the lines from the file.
	 */
	public static Iterable<String> readLines(final File file,
	...
	static class GetLinesIterable implements Iterable<String> {
		...
		// TO DO: better programming style would be to make this two
	 ...
	 *            input reader
	 * @param includeEol
	 *            whether to keep eol-characters in the returned strings
	...
	public static Iterable<String> getLineIterable(Reader r, boolean includeEol) {
		if (includeEol) {
			...
			return new LineReaderIterable(
					(r instanceof BufferedReader) ? (BufferedReader) r
							: new BufferedReader(r));
		}
	}

	public static Iterable<String> getLineIterable(Reader r, int bufferSize,
			boolean includeEol) {
		if (includeEol) {
			...
			return new LineReaderIterable(
					(r instanceof BufferedReader) ? (BufferedReader) r
							: new BufferedReader(r, bufferSize));
		}
	}

	/**
	 * Line iterator that uses BufferedReader.readLine() EOL-characters are
	 * automatically discarded and not included in the strings returns
	 */
	...
	/**
	 * Line iterator that preserves the eol-character exactly as read from
	 * reader. Line endings are: \r\n,\n,\r Lines returns by this iterator will
	 * include the eol-characters
	 **/
	private static final class EolPreservingLineReaderIterable implements
			Iterable<String> {
				...
				private boolean copyUntilEol() {
					for (int i = charBufferPos; i < charsInBuffer; i++) {
						if (charBuffer[i] == '\n') {
							...
							sb.append(charBuffer, charBufferPos, i
									- charBufferPos + 1);
							...
							// not a '\n' here - still need to terminate line
							// (but don't include current character)
							if (i > charBufferPos) {
								sb.append(charBuffer, charBufferPos, i
										- charBufferPos);
								// advance character buffer pos
					...
					sb.append(charBuffer, charBufferPos, charsInBuffer
							- charBufferPos);
	...
	/**
	 * Provides an implementation of closing a file for use in a finally block
	 * so you can correctly close a file without even more exception handling
	 * stuff. From a suggestion in a talk by Josh Bloch.
	 * 
	 * @param c
	...
	public static Iterable<File> iterFilesRecursive(final File dir,
			final String ext) {
		return iterFilesRecursive(dir,
				Pattern.compile(Pattern.quote(ext) + "$"));
	}
	 ...
	 * @param encoding
	 *            The character encoding to assume. This may be null, and the
	 *            platform default character encoding is used.
	 */
	...
	public static String slurpGZippedFile(String filename) throws IOException {
		Reader r = encodedInputStreamReader(new GZIPInputStream(
	 ...
	 * file cannot be read (non-existent, etc.), then the method throws an
	 * unchecked RuntimeIOException. If the caller is willing to tolerate
	 * missing files, they should catch that exception.
	 */
	...
	public static String slurpFileNoExceptions(File file) {
		try {
			return IOUtils.slurpReader(encodedInputStreamReader(
					new FileInputStream(file), null));
		} catch (IOException e) {
			throw new RuntimeIOException(e);
		}
	 ...
	 * @param escapeChar
	 *            - character for escaping quotes appearing in quoted strings;
	 *            defaults to " (i.e. "" is used for " inside quotes, consistent
	 *            with Excel)
	 * @return a list of maps representing the rows of the csv. The maps' keys
	 *         are the header strings and their values are the row contents
	 * @throws IOException
	 */
	public static List<Map<String, String>> readCSVWithHeader(String path,
			char quoteChar, char escapeChar) throws IOException {
		String[] labels = null;
		List<Map<String, String>> rows = Generics.newArrayList();
		for (String line : IOUtils.readLines(path)) {
			System.out.println("Splitting " + line);
			if (labels == null) {
				labels = StringUtils.splitOnCharWithQuoting(line, ',', '"',
						escapeChar);
			} else {
				String[] cells = StringUtils.splitOnCharWithQuoting(line, ',',
	...
	/**
	 * Read a CSV file character by character. Allows for multi-line CSV files
	 * (in quotes), but is less flexible and likely slower than
	 * readCSVWithHeader()
	 * 
	 * @param csvContents
	 *            The char[] array corresponding to the contents of the file
	 * @param numColumns
	 *            The number of columns in the file (for verification,
	 *            primarily)
	 * @return A list of lines in the file
	 */
	public static LinkedList<String[]> readCSVStrictly(char[] csvContents,
			int numColumns) {
		// --Variables
	...
	public static LinkedList<String[]> readCSVStrictly(String filename,
			int numColumns) throws IOException {
		return readCSVStrictly(slurpFile(filename).toCharArray(), numColumns);
	}

	/**
	 * Get a input file stream (automatically gunzip/bunzip2 depending on file
	 * extension)
	 * 
	 * @param filename
	 ...
	 * @throws IOException
	 *             if there are exceptions opening the file
	 */
	...
	/**
	 * Get a output file stream (automatically gzip/bzip2 depending on file
	 * extension)
	 * 
	 * @param filename
	 *            Name of file to open
	...
	public static BufferedReader getBufferedReaderFromClasspathOrFileSystem(
			String filename) throws IOException {
		return getBufferedReaderFromClasspathOrFileSystem(filename,
				defaultEncoding);
	}
	...
	public static PrintWriter getPrintWriter(File textFile, String encoding)
		...
		return new PrintWriter(new BufferedWriter(new OutputStreamWriter(
				new FileOutputStream(f), encoding)), true);
	}
	...
	public static InputStream getBZip2PipedInputStream(String filename)
		...
		Writer errWriter = new BufferedWriter(
				new OutputStreamWriter(System.err));
		StreamGobbler errGobler = new StreamGobbler(p.getErrorStream(),
				errWriter);
		errGobler.start();
	 ...
	 * @throws IOException
	 */
	public static Set<String> readColumnSet(String infile, int field)
			throws IOException {
	...
	public static <C> List<C> readObjectFromColumns(Class objClass,
		...
		while ((line = br.readLine()) != null) {
			line = line.trim();
			if (line.length() > 0) {
				C item = StringUtils.columnStringToObject(objClass, line,
						delimiterPattern, fieldNames);
				list.add(item);
			}
		}
		br.close();
	 ...
	 * empty, if the file is empty. If there is an IOException, it is caught and
	 * null is returned.
	 */
	 ...
	 * Returns the contents of a file as a single string. The string may be
	 * empty, if the file is empty. If there is an IOException, it is caught and
	 * null is returned. Encoding can also be specified.
	 */
	public static String stringFromFile(String filename, String encoding) {
		try {
			StringBuilder sb = new StringBuilder();
			BufferedReader in = new BufferedReader(new EncodingFileReader(
					filename, encoding));
			String line;
	...
	/**
	 * A JavaNLP specific convenience routine for obtaining the current scratch
	 * directory for the machine you're currently running on.
	 */
	...
	/**
	 * Given a filepath, makes sure a directory exists there. If not, creates
	 * and returns it. Same as ENSURE-DIRECTORY in CL.
	 * 
	 * @param tgtDir
	 *            The directory that you wish to ensure exists
	 * @throws IOException
	 *             If directory can't be created, is an existing file, or for
	 *             other reasons
	 */
		...
		if (tgtDir.exists()) {
			if (tgtDir.isDirectory()) {
				...
				throw new IOException("Could not create directory "
						...
						+ ", as a file already exists at that path.");
			}
		} else {
			if (!tgtDir.mkdirs()) {
				throw new IOException("Could not create directory "
	...
	/**
	 * Create a Reader with an explicit encoding around an InputStream. This
	 * static method will treat null as meaning to use the platform default,
	 ...
	 *             If any IO problem
	 */
	public static Writer encodedOutputStreamWriter(OutputStream stream,
			String encoding) throws IOException {
	 ...
	 *             If any IO problem
	 */
	public static PrintWriter encodedOutputStreamPrintWriter(
			OutputStream stream, String encoding, boolean autoFlush)
			throws IOException {
		// PrintWriter doesn't allow encoding to be null; or to have charset and
		// flush
		if (encoding == null) {
	...
	/**
	 * A raw file copy function -- this is not public since no error checks are
	 * made as to the consistency of the filed being copied. Use instead:
	 * 
	 * @see IOUtils#cp(java.io.File, java.io.File, boolean)
	 * @param source
	 *            The source file. This is guaranteed to exist, and is
	 *            guaranteed to be a file.
	 * @param target
	 *            The target file.
	 * @throws IOException
	 *             Throws an exception if the copy fails.
	 */
	...
	/**
	 * <p>
	 * An implementation of cp, as close to the Unix command as possible. Both
	 * directories and files are valid for either the source or the target; if
	 * the target exists, the semantics of Unix cp are [intended to be] obeyed.
	 * </p>
	 * 
	 * @param source
	 *            The source file or directory.
	 * @param target
	 *            The target to write this file or directory to.
	 * @param recursive
	 *            If true, recursively copy directory contents
	 * @throws IOException
	 *             If either the copy fails (standard IO Exception), or the
	 *             command is invalid (e.g., copying a directory without the
	 *             recursive flag)
	 */
	public static void cp(File source, File target, boolean recursive)
			throws IOException {
		// Error checks
		...
		if (!target.getParentFile().exists()) {
			...
			throw new IOException("cp: cannot copy to directory: " + recursive
		...
		if (target.exists() && target.isDirectory()) {
			trueTarget = new File(target.getPath() + File.separator
					+ source.getName());
		} else {
			trueTarget = target;
		}
		...
		if (source.isFile()) {
			...
			if (children == null) {
				throw new IOException("cp: could not list files in source: "
						+ source);
			}
			...
			if (target.exists()) {
				...
				if (!trueTarget.exists() && !trueTarget.mkdir()) {
					...
					throw new IOException("cp: could not create directory: "
							+ trueTarget);
				}
			} else {
			...
			for (File child : children) {
				File childTarget = new File(trueTarget.getPath()
