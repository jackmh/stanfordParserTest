 ...
 * <p>
 * Tokenization: The default tokenizer is {@link PTBTokenizer}. If null is
 * passed to <code>setTokenizerFactory</code>, then whitespace tokenization is
 * assumed.
 * <p>
 * Adding a new document type requires two steps:
 * <ol>
 * <li>Add a new DocType.
 * <li>Create an iterator for the new DocType and modify the iterator() function
 * to return the new iterator.
 * </ol>
 * <p>
...
public class DocumentPreprocessor implements Iterable<List<HasWord>> {
	...
	private TokenizerFactory<? extends HasWord> tokenizerFactory = PTBTokenizer
			.coreLabelFactory();
	...
	/**
	 * Example: if the words are already POS tagged and look like foo_VB, you
	 * want to set the tagDelimiter to "_"
	 */
	private String tagDelimiter = null;
	/**
	 * When doing XML parsing, only accept text in between tags that match this
	 * regular expression. Defaults to everything.
	 */
	...
	/**
	 * Constructs a preprocessor from a file at a path, which can be either a
	 * filesystem location, a classpath entry, or a URL.
	 * 
	 * @param docPath
	 *            The path
	 * @param encoding
	...
	/**
	 * Set whether or not the tokenizer keeps empty sentences in whitespace
	 * mode. Useful for programs that want to echo blank lines. Not relevant for
	 * the non-whitespace model.
	 */
	...
	/**
	 * Sets the factory from which to produce a {@link Tokenizer}. The default
	 * is {@link PTBTokenizer}.
	 * <p>
	 * NOTE: If a null argument is used, then the document is assumed to be
	 * tokenized and DocumentPreprocessor performs no tokenization.
	 * 
	 */
	public void setTokenizerFactory(
			TokenizerFactory<? extends HasWord> newTokenizerFactory) {
	...
	/**
	 * Make the processor assume that the document is already delimited by the
	 * supplied parameter.
	 * 
	 * @param s
	 ...
	 * the returned label.
	 * <p>
	 * Note that for strings that contain two or more instances of the tag
	 * delimiter, the last instance is treated as the split point.
	 * <p>
	 * The tag delimiter should not contain any characters that must be escaped
	 * in a Java regex.
	 * 
	 * @param s
	 *            POS tag delimiter
	...
	/**
	 * Returns sentences until the document is exhausted. Calls close() if the
	 * end of the document is reached. Otherwise, the user is required to close
	 * the stream.
	 */
	...
	private class PlainTextIterator implements Iterator<List<HasWord>> {
		...
		public PlainTextIterator() {
			...
			if (sentenceDelimiter == null) {
				...
				delimFollowers = Generics.newHashSet(Arrays
						.asList(sentenceFinalFollowers));
			} else {
				sentDelims.add(sentenceDelimiter);
				delimFollowers = Generics.newHashSet();
				eolIsSignificant = wsPattern.matcher(sentenceDelimiter)
						.matches();
			...
			if (tokenizerFactory == null) {
				...
				tokenizer = WhitespaceTokenizer.newWordWhitespaceTokenizer(
						inputReader, eolIsSignificant);
			} else {
				if (eolIsSignificant) {
			...
			// Note that if the token contains two or more instances of the
			// delimiter, then the last
			// instance is regarded as the split point.
			if (tagDelimiter != null) {
				splitTag = new Function<String, String[]>() {
					private final String splitRegex = String.format(
							"%s(?!.*%s)", tagDelimiter, tagDelimiter);
			...
			do {
				...
				if (splitTag != null) {
					...
					if (toks.length == 2 && token instanceof HasTag) {
						...
						// JavaNLP labels. We could convert to CoreLabel here,
						// but
				...
				if (sentDelims.contains(token.word())) {
					seenBoundary = true;
						...
						&& !delimFollowers.contains(token.word())) {
					nextSentCarryover.add(token);
					break;
				}

				if (!(wsPattern.matcher(token.word()).matches() || token.word()
						.equals(PTBLexer.NEWLINE_TOKEN))) {
					nextSent.add(token);
				}
	...
	private class XMLIterator implements Iterator<List<HasWord>> {
		...
		public XMLIterator() {
			xmlItr = new XMLBeginEndIterator<String>(inputReader,
					elementDelimiter);
	 ...
	 * Usage: java DocumentPreprocessor filename [-xml tag] [-suppressEscaping]
	 * [-noTokenization]
	 * <p>
	 * A filename is required. The code doesn't run as a filter currently.
	 * <p>
	 * tag is the element name of the XML from which to extract text. It can be
	 * a regular expression which is called on the element with the matches()
	 * method, such as 'TITLE|P'.
	 * 
	 * @param args
	...
	public static void main(String[] args) throws IOException {
		if (args.length < 1) {
			...
			System.err
					.println("    OPT = -xml TAG|-encoding ENC|-tokenizerOptions opts|-tag delim|...");
			return;
		}
		...
		for (; i < args.length; i++) {
			...
			if (args[i].equals("-xml")) {
			...
			} else if (args[i].equals("-tokenizerOptions")
					&& i + 1 < args.length) {
				i++;
				...
				tf = null;
				List<String> whitespaceDelims = new ArrayList<String>(
						Arrays.asList(DocumentPreprocessor.DEFAULT_SENTENCE_DELIMS));
				whitespaceDelims.add(WhitespaceLexer.NEWLINE);
				sentenceDelims = whitespaceDelims
						.toArray(new String[whitespaceDelims.size()]);
