...
public class ppiTextProcess {
	...
	/*
	 * 1. 对摘要进行分句; 2. 针对摘要中的每一句, 识别出其中的蛋白质并对蛋白质进行标准化 3. 对每一句话应用Stanford
	 * Parser进行解析, 得到语法树 4. 由3得到的语法树, 结合关系词、gene库找到候选的蛋白质相互作用对
	 */
	...
	public void pubmedTextProcessing(String pubmedTextFullName,
			HashSet<String> geneSet, HashSet<String> allKeysSets,
		...
		/*********************************************************/
		/*
		 * 1. Convert the Abstract text into sentenceList. each list contain of
		 * sentence list. 2. In each paragraph, split each sentence with
		 * char[.?!] as default.
		 */
		...
		for (String paragraph : allLines) {
			...
			for (List<HasWord> sentence : sentenceListWord) {
				 ...
				 * this sentece into several part.
				 */
				if (sentence.size() > RADIXNUM) {
					int k = 0, i = 0;
					int num = (sentence.size() + RADIXNUM - 1) / RADIXNUM; 
					while (k < num) {
						i = 0;
						List<HasWord> sentTmp = new LinkedList<HasWord>();
						while (i < RADIXNUM
								&& (i + k * RADIXNUM) < sentence.size()) {
							HasWord tmpHasWord = sentence.get(i + k * RADIXNUM);
				...
				for (List<HasWord> sent : sentListTmp) {
					proteinREC proteinSentRec = new proteinREC();
					proteinSentRec.proteinRecognition(sent, allKeysSets,
							relationKeySet, firstCharDict, geneSynProteinDict);
					newPubmedText += proteinSentRec.getNewSentenceStr() + " ";
					if (config.__DEBUG__ == true) {
						System.out.println(proteinSentRec
								.getNumberOfRecognitionProteins()
								+ "\t"
								+ proteinSentRec.getNumberOfRelationWords()
								+ "\n"
					...
					if (proteinSentRec.getNumberOfRecognitionProteins() >= 2 && proteinSentRec.getNumberOfRelationWords() > 0) {
						...
						if (proteinRelationExtracTree
								.getNumberOfInteractionPair() > 0) {
							HashSet<String> sentenceInteracticePairSet = proteinRelationExtracTree
		...
		setTextOfInteractiveProtein(getInteractiveProteinText(
				interactiveProteinPairSet, newTextOfRecProtein));
		if (config.__WriteIntoFileFlag__ == true) {
			writePubmedTextIntoFile();
		}
	}

	private String getInteractiveProteinText(
			HashSet<String> interactiveProteinPairSet,
			String newTextOfRecProtein) {
